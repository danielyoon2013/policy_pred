...
enactment) is visible despite policies sitting at different P(yes) baselines.
...
unmapped = sorted({b for p, b in
                   ((p, str(bat.loc[bat.event_id == p, "domain"].iloc[0]).split("_")[0]) for p in pid2cat)
                   if pid2cat[p] == "UNMAPPED"})
...
for r in rows:
    r["rel"], r["p"] = int(r["rel_year"]), float(r["p_yes"])
    r["cat"] = pid2cat.get(r["policy_id"], "UNMAPPED")

XS = [5000, 10000, 20000]
RELS = list(range(-10, 11))

def per_policy_z(recs):
    """{rel: [z...]} aggregating per-policy z-scores (center+scale by policy's own mean/std, >=5 yr)."""
...
for x in XS:
    ax.plot(RELS, curve(per_policy_z(byX[x])), "-o", ms=4, color=colors[x], label=f"X={x//1000}k")
ax.axvline(0, color="k", lw=1, ls="--", alpha=.6, label="enactment")
...
ax.set_title("nanochat LoRA-only: per-policy z-normalized P(yes)\nvs years-to-enactment (211 policies, 1931-2020)")
ax.grid(alpha=.25); ax.legend(title="data/yr")
fig.tight_layout(); fig.savefig(HERE / "figures" / "nshape_znorm.png", dpi=130); plt.close(fig)
...
# =================== FIG 3: per-domain pre-enactment z-rise bar ===================
fig, ax = plt.subplots(figsize=(11, 6))
incs = [cat_data[c]["inc"] for c in ordered]; ns = [cat_data[c]["npol"] for c in ordered]
ax.barh(range(len(ordered)), incs, color=["#2ca02c" if v > 0 else "#d62728" for v in incs])
ax.set_yticks(range(len(ordered))); ax.set_yticklabels([f"{c}  (n={n})" for c, n in zip(ordered, ns)], fontsize=9)
ax.invert_yaxis(); ax.axvline(0, color="k", lw=1)
ax.set_xlabel("pre-enactment z-rise (rel -10 -> 0), X=10k")
ax.set_title("nanochat: per-policy z-normalized pre-enactment rise by policy domain\n"
             "(positive = P(yes) rises approaching enactment)", fontsize=11)
...
print("\nper-domain z-normalized pre-enactment rise (X=10k), ranked:")
for c in ordered:
    print(f"  {c:<26} n={cat_data[c]['npol']:>2}  rise={cat_data[c]['inc']:+.3f}z")
